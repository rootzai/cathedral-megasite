...
export default function UnifiedLayout({
    ...
    return (
        <div className={cn(`min-h-screen flex flex-col pt-20`, themeClass)} style={{ background: "var(--background)", color: "var(--foreground)" }}>
            {/* Header Section */}
            <header className="w-full border-b border-border bg-background/95 backdrop-blur-md pt-10 pb-6 px-4 md:px-8 z-40 sticky top-20 shadow-sm">
                ...
                <div className="max-w-6xl mx-auto">
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                        <div className="flex flex-col gap-2">
                            ...
                            <h1 className="font-heading text-4xl md:text-6xl text-foreground uppercase tracking-tight leading-[0.9] mb-3">
                                {title}
                            </h1>
                            <p className="font-serif italic text-muted-foreground text-base md:text-lg max-w-2xl leading-relaxed">
                                {subtitle}
                            </p>
                        ...
                        <div className="flex items-center gap-2">
                            <Link href="/">
                                <div className="font-mono text-xs uppercase tracking-[0.2em] text-foreground font-bold hover:text-destructive cursor-pointer transition-all border-2 border-foreground/10 px-6 py-2 rounded-sm hover:border-destructive hover:bg-destructive/5 active:scale-95 shadow-sm">
                                    Return to Archway
                                </div>
                        ...
                        {navItems.map((item) => {
                            ...
                            return (
                                <Link key={item.id} href={item.path}>
                                    <div className={cn(
                                        "flex-shrink-0 py-4 px-8 text-center font-mono text-[11px] md:text-sm uppercase tracking-[0.15em] cursor-pointer transition-all border-r border-border hover:bg-foreground/5 last:border-r-0 whitespace-nowrap",
                                        isActive
