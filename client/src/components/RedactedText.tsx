...
export default function RedactedText({ children, className, reveal = true }: RedactedTextProps) {
  return (
    <span 
      className={cn(
        "relative inline-block px-1 mx-0.5 bg-white text-zinc-900 transition-all duration-700 ease-in-out cursor-help group",
    ...
    >
      ...
      {reveal && (
        <span className="absolute inset-0 bg-white group-hover:w-0 transition-all duration-500 ease-out" />
      )}
    </span>
  );
}
