...
export function FirmCard({ firm, index = 0 }: { firm: LawyerFirm; index?: number }) {
  return (
    <motion.div
      ...
      className="relative p-5 rounded-sm gold-border gold-glow bg-[oklch(0.1_0.005_285)] hover:bg-[oklch(0.12_0.005_285)] transition-all duration-300"
    >
      {/* Firm Name */}
      ...
      <div className="space-y-3 ml-2">
        {firm.attorneys.map((attorney) => (
          <div key={attorney.name} className="border-l-2 border-[oklch(0.75_0.12_85/25%)] pl-4">
      ...
      {/* Firm Description */}
      <p className="text-sm text-muted-foreground leading-relaxed mt-4 pt-3 border-t border-[oklch(0.75_0.12_85/10%)]">
        {firm.description}
      </p>
    </motion.div>
  );
