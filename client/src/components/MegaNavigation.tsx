...
export default function MegaNavigation() {
  ...
  const navLinkClass = (path: string) => {
    ...
    return `
      px-2 lg:px-4 py-3 text-xs lg:text-xs font-bold uppercase tracking-[0.2em] transition-all text-center
      ${isLinkActive
      ...
      <div className="w-full px-2 lg:px-6">
        ...
        <div className="hidden md:flex items-center justify-between h-16">
          <div className="flex items-center justify-between w-full h-full">
            ...
            <Link href="/tips">
                <a className="ml-4 border border-red-900/50 hover:bg-red-600 text-white px-6 py-2.5 text-xs font-black uppercase tracking-[0.4em] transition-all shadow-[0_0_20px_rgba(139,26,26,0.3)] hover:shadow-[0_0_30px_rgba(220,38,38,0.5)] flex items-center justify-center">
        ...
        <div className="flex animate-ticker items-center py-1">
          {[...newsItems, ...newsItems].map((item, idx) => (
            <div key={idx} className="flex items-center px-12">
              <span className={`text-xs font-bold px-3 py-1 mr-3 rounded border ${item.type === 'BREAKING' ? 'bg-red-900 text-white border-red-600' : 'bg-zinc-900 text-zinc-100 border-white/10'}`}>
                {item.type}
              </span>
        ...
        <div className="bg-zinc-950 border-t border-red-900/20">
          <div className="container mx-auto px-4 py-1.5 flex flex-wrap justify-center gap-8">
            {['Who Knew', 'Regents', 'Trustees', 'Lawyers', 'Timeline'].map(tab => (
              <a key={tab} href={`#${tab.toLowerCase()}`} className="text-xs font-bold uppercase tracking-widest text-zinc-500 hover:text-red-500 transition-colors">{tab}</a>
            ))}
          </div>
        </div>
      )}
