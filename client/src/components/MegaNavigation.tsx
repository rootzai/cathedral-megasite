  ...
  return (
    <nav aria-label="Primary Navigation" className="bg-black border-b border-white/5 relative z-50">
      ...
      <div className="w-full px-4 lg:px-8">
        <div className="hidden md:flex items-center justify-between h-20">
          <Link href="/" className="h-full flex items-center group transition-all shrink-0">
            <img src="/assets/images/colored-pirate-logo.png" alt="Sodom Hall Home" className="w-12 h-auto object-contain group-hover:scale-110 transition-all filter grayscale brightness-50 contrast-125" />
            <span className="ml-4 text-zinc-500 group-hover:text-zinc-100 transition-colors duration-500 font-cinzel font-black tracking-[0.3em] text-sm uppercase">Sodom Hall</span>
          </Link>

          <div className="flex gap-4">
              <Link href="/evidence" className="border border-red-900 bg-red-900/10 hover:bg-red-900/30 text-red-500 hover:text-red-400 px-8 py-3 text-sm font-black uppercase tracking-[0.4em] transition-all shadow-[0_0_20px_rgba(139,26,26,0.1)] hover:shadow-[0_0_30px_rgba(139,26,26,0.3)] flex items-center justify-center gap-3">
              ...
              <Link href="/tips" className="border border-white/10 hover:border-white/30 text-zinc-400 hover:text-white px-6 py-3 text-xs font-black uppercase tracking-[0.3em] transition-all flex items-center justify-center">
                  Tip Line
              </Link>
          </div>
        </div>
        ...
        <div className="md:hidden">
          <div className="flex items-center justify-between py-4">
            <Link href="/" className="flex items-center gap-3">
              <img src="/assets/images/colored-pirate-logo.png" alt="Sodom Hall Home" className="h-10 w-auto object-contain filter grayscale" />
            </Link>
            <Link href="/evidence" className="bg-red-900/20 border border-red-900/50 text-red-500 px-4 py-2 text-xs font-bold uppercase tracking-widest flex items-center gap-2">
      ...
      <div className="hidden md:flex bg-[#080808] h-8 items-center border-t border-white/5 group" aria-label="News ticker" role="region">
        <button onClick={() => setTickerPaused(!tickerPaused)} className="shrink-0 px-3 h-full flex items-center text-zinc-600 hover:text-white transition-colors border-r border-white/5 cursor-pointer z-10">
          {tickerPaused ? <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg> : <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><rect x="6" y="4" width="4" height="16" /><rect x="14" y="4" width="4" height="16" /></svg>}
        </button>
          ...
          <div className={`flex ${tickerPaused ? '' : 'animate-ticker'} group-hover:[animation-play-state:paused] items-center whitespace-nowrap`} style={{ minWidth: 'max-content', ...(tickerPaused ? { animationPlayState: 'paused' } : {}) }}>
            {[...newsItems, ...newsItems].map((item, idx) => (
              <div key={idx} className="flex items-center px-12 shrink-0">
                <span className={`text-xs font-bold px-2 py-0.5 mr-3 rounded border shrink-0 ${item.type === 'BREAKING' ? 'bg-zinc-900 text-red-500 border-red-900/50' : 'bg-black text-zinc-500 border-white/5'}`}>{item.type}</span>
                <span className="text-xs font-mono text-zinc-400 uppercase tracking-widest">{item.text}</span>
              </div>
            ))}
          </div>
        </div>
